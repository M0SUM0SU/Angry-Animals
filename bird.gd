...
enum AnimalState {Ready, Drag, Relase}
...
@onready var stretch_s: AudioStreamPlayer2D = $StretchS
@onready var launch_s: AudioStreamPlayer2D = $LaunchS
@onready var kick_s: AudioStreamPlayer2D = $KickS
...
func _unhandled_input(event: InputEvent) -> void:
	if _state == AnimalState.Drag and event.is_action_released("drag"):
		change_state(AnimalState.Relase)
		call_deferred("change_state", AnimalState.Relase)
...
func handle_dragging() -> void:
	...
	if diff.length() > 0 and stretch_s.playing == false:
		stretch_s.play()
...
func _on_body_entered(body: Node) -> void:
	if body is Cup and kick_s.playing == false:
		kick_s.position = body.position
		kick_s.play()
